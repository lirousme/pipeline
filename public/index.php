...
function closeConditionalModal(){conditionalModal.classList.add('hidden');nextProblemSearch.value='';searchSuggestions.innerHTML='';selectedProblem=null;selectedProblemInfo.classList.add('hidden');conditionalFatherId=null;}
...
saveConditional.onclick=async()=>{const searchValue=nextProblemSearch.value.trim();if(!conditionalFatherId||searchValue==='')return;const payload={id_father_problem:conditionalFatherId,text:searchValue};if(selectedProblem){payload.id_next_problem=Number(selectedProblem.id);}else{payload.next_problem_text=searchValue;}const r=await fetch(api+'?action=add-conditional',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});if(r.ok){closeConditionalModal();loadProblems();openPipeline(conditionalFatherId);}};
async function loadProblems(){const r=await fetch(api+'?action=list-problems');const d=await r.json();problemList.innerHTML='';(d.items||[]).forEach(p=>{const li=document.createElement('li');li.className='bg-slate-900 border border-white/10 p-3 rounded';li.innerHTML=`<div class='font-medium'>#${p.id} ${p.text}</div><div class='mt-2 flex gap-2'><a class='bg-indigo-600 rounded px-2 py-1 text-sm inline-block' href='./pipeline.php?problem_id=${p.id}'>Abrir pipeline</a><button class='bg-slate-700 rounded px-2 py-1 text-sm' onclick='addConditional(${p.id})'>+ Condicional</button></div>`;problemList.appendChild(li);});}
