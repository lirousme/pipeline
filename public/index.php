...
async function addConditional(idFather){const next=prompt('ID do próximo problema');const text=prompt('Texto da condicional');if(!next||!text)return;await fetch(api+'?action=add-conditional',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id_father_problem:idFather,id_next_problem:Number(next),text})});}
loadProblems();
</script>
<?php endif; ?></body></html>
